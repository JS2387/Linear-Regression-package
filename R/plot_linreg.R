...
plot.linreg <- function (x, y, ...) {
                          ...
                          plot_res_vs_fitted <- ggplot2::ggplot(df_1 , aes(x=y_hat , y=error_hat) )+
                          ...
                          plot_scale_location  <- ggplot2::ggplot(df_2 , aes(x=y_hat , y= standard_res_sqrt ))+
                            geom_point(shape = 1 , size=4) +
                            geom_line(aes(x = smoothed_2$x, y = smoothed_2$y), color = "red") +
                            ggtitle("Scale-Location") +
                            theme(plot.title = element_text(hjust = 0.5))+
                            ylab("Sqrt(Standardised Residuals)")+
                            xlab("Fitted values")+
                            theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                                  panel.background = element_blank(), axis.line = element_line(colour = "black")) +
                            theme(plot.margin=unit(c(2,2,2.5,1.2),"cm"))
